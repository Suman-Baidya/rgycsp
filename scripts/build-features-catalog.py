#!/usr/bin/env python3
"""
ABCD Edu Hub — Complete Platform Features & Capabilities Catalog Builder
...
# Executive Color Palette (Emerald / Teal / Dark Slate)
...
def set_callout_border(cell, color_hex="0D9488"):
...
def create_callout_box(doc, callout_type, content_lines):
    ...
    colors = {
        'NOTE': ('F0FDF4', '0D9488', 'ENTERPRISE HIGHLIGHT: Architecture & Topology'),
        'TIP': ('F0FDF4', '10B981', 'CLIENT VALUE PROPOSITION: Business Benefit'),
        'IMPORTANT': ('FEF2F2', 'EF4444', 'CRITICAL CAPABILITY: Enterprise Grade'),
        'WARNING': ('FFFBEB', 'F59E0B', 'SECURITY GOVERNANCE: Compliance Standard'),
        'CAUTION': ('FFF1F2', 'E11D48', 'OPERATIONAL CONSTRAINT: Integrity Safeguard')
    }
    
    bg_hex, border_hex, header_title = colors.get(callout_type, ('F8FAFC', '0D9488', 'SYSTEM CAPABILITY'))
...
def build_cover_page(doc):
    ...
    r_badge = p_badge.add_run("ABCD EDU HUB — ENTERPRISE PRODUCT & CAPABILITIES DOSSIER")
    ...
    r_title = p_title.add_run("Complete Platform Features\n& Capabilities Catalog")
    ...
    r_sub = p_sub.add_run("The Technical & Operational Blueprint for Enterprise Education Hubs, Multi-Branch Franchise Networks, and Accredited National Boards")
    ...
    set_callout_border(cell, "065F46")
    
    cp = cell.paragraphs[0]
    cprun = cp.add_run("EXECUTIVE PLATFORM HIGHLIGHTS AT A GLANCE:")
    cprun.bold = True
    ...
    highlights = [
        "Multi-Tenant Dual-Routing Engine (Seamless Subdomain & Subdirectory Modes)",
        "Hardware-Level Biometric Passkeys (Face ID, Fingerprint, Windows Hello, Touch ID)",
        "Central Board Command Center: Franchise Licensing, Impersonation & Audit",
        "Dynamic Course Pricing & Multi-Tier Installment (EMI) Billing Engine",
        "Customizable Online Admission Form Builder with Dynamic Custom Questions",
        "One-Click Student Enrollment & Automated Standardized Accreditation Numbering",
        "Fees Management: Cash Collections, Print Receipts & Online Student UPI Slip Audits",
        "Classroom Attendance Register with 4-State Tracking & Monthly Percentage Reports",
        "Visual Document Designer for PVC ID Cards, Accredited Certificates & Marksheets",
        "Public Instant Verification Engine with Tamper-Proof Anti-Counterfeit QR Codes",
        "Dedicated Mobile-First Student Portal with Syllabus & Financial Ledgers",
        "Pre-Paid Internal Token Economy for Central Service Automation"
    ]
    for h in highlights:
    ...
    meta_table.rows[0].cells[0].paragraphs[0].add_run("Edition: Client Presentation & RFP Edition").font.size = Pt(9)
    meta_table.rows[0].cells[1].paragraphs[0].add_run("Platform Release: Academic Year 2026-2027").font.size = Pt(9)
    meta_table.rows[1].cells[0].paragraphs[0].add_run("Target Audience: Investors, Boards & Franchisees").font.size = Pt(9)
    meta_table.rows[1].cells[1].paragraphs[0].add_run("Security Rating: Enterprise Grade (Zero-Trust)").font.size = Pt(9)
...
def main():
    print("Initializing ABCD Edu Hub Features Catalog Document Generator...")
    doc = Document()
    ...
    hrun = hp.add_run("ABCD Edu Hub — Complete Platform Features & Capabilities Catalog")
    ...
    frun = fp.add_run("Client Presentation Edition — Proprietary Educational Management Suite")
    ...
    chapters_dir = os.path.join(os.path.dirname(__file__), '..', 'docs', 'user-guides', 'features-catalog')
    ...
    output_path = os.path.join(output_dir, 'ABCD_Edu_Hub_Features_Catalog.docx')
