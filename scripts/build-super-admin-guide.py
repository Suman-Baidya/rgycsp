#!/usr/bin/env python3
"""
ABCD Edu Hub — Super Admin Operational Guide Builder
...
# Color Palette for Super Admin (Executive Royal / Crimson Navy)
COLOR_PRIMARY = RGBColor(15, 23, 42)       # Slate 900 #0F172A
COLOR_SECONDARY = RGBColor(30, 58, 138)   # Deep Navy #1E3A8A
...
def set_callout_border(cell, color_hex="4F46E5"):
...
def create_callout_box(doc, callout_type, content_lines):
    ...
    colors = {
        'NOTE': ('F8FAFC', '4F46E5', 'NOTE: Governance Context'),
        'TIP': ('F0FDF4', '10B981', 'EXECUTIVE TIP: Operational Efficiency'),
        'IMPORTANT': ('FEF2F2', 'EF4444', 'CRITICAL: Central Accreditation Rule'),
        'WARNING': ('FFFBEB', 'F59E0B', 'SECURITY WARNING: Authority Constraint'),
        'CAUTION': ('FFF1F2', 'E11D48', 'CAUTION: Master Privileges Required')
    }
    
    bg_hex, border_hex, header_title = colors.get(callout_type, ('F8FAFC', '4F46E5', 'INFORMATION'))
...
def build_cover_page(doc):
    ...
    r_badge = p_badge.add_run("ABCD EDU HUB — CENTRAL HEAD OFFICE ADMINISTRATION")
    ...
    r_title = p_title.add_run("Super Administrator\nMaster Operational Manual")
    ...
    r_sub = p_sub.add_run("The Complete Handbook for Platform Governance, Franchise Licensing, Nationwide Student Oversight, Token Economy, and Accreditation Standards")
    ...
    set_callout_border(cell, "0F172A")
    
    cp = cell.paragraphs[0]
    cprun = cp.add_run("EXECUTIVE GOVERNANCE AREAS COVERED IN THIS VOLUME:")
    cprun.bold = True
    ...
    modules = [
        "Platform Architecture, Central Hierarchy & Multi-Tenant Routing Mechanics",
        "Franchise Affiliation Management: Setup, Provisioning, Licensing & Subdomains",
        "Understanding Franchise Student Enrolment Workflows & Central Registry Audits",
        "Central Student Registration Card Approvals & Tamper-Proof Document Verification",
        "Global Accredited Course Master, Curriculum Standardization & Group Taxonomy",
        "Central Token Economy, Merchant Banking Setup & Franchise Top-Up Reconciliations",
        "Document Designer: ID Cards, Accredited Certificates & Marksheets with QR Codes",
        "Global Identity Management & Super Admin Manager Delegation Framework",
        "Regional State Manager Coordinator Network & Territorial Jurisdiction Controls",
        "Central Wholesale Merchandise Logistics, Textbook Bundles & Order Processing",
        "National Platform Settings, Central Landing Page & Immutable Security Audit Logs"
    ]
    for m in modules:
    ...
    meta_table.rows[0].cells[0].paragraphs[0].add_run("Document Version: 3.0 (Master HQ Edition)").font.size = Pt(9)
    meta_table.rows[0].cells[1].paragraphs[0].add_run("Release Date: Academic Year 2026-2027").font.size = Pt(9)
    meta_table.rows[1].cells[0].paragraphs[0].add_run("Target Audience: Executive Directors & HQ Staff").font.size = Pt(9)
    meta_table.rows[1].cells[1].paragraphs[0].add_run("Classification: Confidential & Proprietary").font.size = Pt(9)
...
def main():
    print("Initializing ABCD Edu Hub Super Admin Guide Document Generator...")
    doc = Document()
    ...
    hrun = hp.add_run("ABCD Edu Hub — Super Administrator Master Operational Manual")
    ...
    frun = fp.add_run("Strictly Confidential — Executive Head Office Operations Only")
    ...
    chapters_dir = os.path.join(os.path.dirname(__file__), '..', 'docs', 'user-guides', 'super-admin')
    ...
    output_path = os.path.join(output_dir, 'ABCD_Edu_Hub_Super_Admin_Guide.docx')
