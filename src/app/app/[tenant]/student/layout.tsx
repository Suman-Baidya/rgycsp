...
export default async function StudentLayout({
  ...
  if (!session) {
    redirect(`/login?callbackUrl=/app/${tenant}/student/dashboard`);
  }
    ...
    <div className="flex min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden pb-24 lg:pb-0">
        <header className="h-20 border-b border-border/40 bg-background/50 backdrop-blur-md flex items-center px-4 lg:px-8 sticky top-0 z-40">
          <div className="lg:hidden ml-14 font-black tracking-tight text-lg text-foreground uppercase truncate max-w-[200px]">
            {workspace.name}
          </div>
          <div className="hidden lg:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
          ...
          <div className="ml-auto flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-border">
               <div className="text-right hidden sm:block">
                  <p className="text-[10px] font-black uppercase tracking-widest text-foreground">{session.user.name}</p>
                  <p className="text-[8px] font-bold text-primary uppercase tracking-widest opacity-70">Enrolled Student</p>
               </div>
               <div className="h-10 w-10 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center text-xs font-black text-primary shadow-inner">
