  ...
  const testimonials = content.items || [
    {
      ...
      content: "The faculty here is amazing. They don't just teach the syllabus but also prepare you for the real world challenges. I got placed in a top MNC thanks to the guidance I received.",
    ...
    },
    {
      name: "Priya Patel",
      role: "MBA Student",
      content: "Excellent infrastructure and very supportive environment. The practical exposure I got here was invaluable for my career growth.",
    ...
    },
    {
      ...
      content: "The training programs are very well structured. I learned a lot of technical skills that helped me secure a great job right after my diploma.",
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-20">
          <div className="max-w-2xl space-y-4 text-left">
            ...
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">
              Inspiring Future <span className="text-primary">Generations</span>
            </h2>
          </div>
            ...
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-background overflow-hidden relative">
                   <Image src={`https://i.pravatar.cc/100?u=${i}`} alt="avatar" fill />
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t: any, i: number) => (
            <div key={i} className="group p-10 rounded-[3rem] bg-zinc-50 dark:bg-zinc-900 border border-border/40 hover:border-primary/30 hover:bg-white dark:hover:bg-zinc-800 transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5 relative">
              ...
              <div className="space-y-8 relative z-10">
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, idx) => (
                    <Star key={idx} className={cn("w-4 h-4", idx < t.rating ? "text-orange-400 fill-orange-400" : "text-zinc-300")} />
                  ))}
                </div>
                ...
                <div className="pt-8 border-t border-border/50 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-bold text-primary uppercase tracking-widest">{t.course}</div>
                  </div>
                </div>
              </div>
            </div>
