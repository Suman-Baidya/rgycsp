...
export function WorkspaceHero({ data }: { data: any }) {
  ...
  const slides = (data?.content?.slides && data.content.slides.length > 0) ? data.content.slides : [
    ...
    {
      banner: "https://cdn.pixabay.com/photo/2016/11/29/11/52/computer-1869306_1280.jpg",
      tagline: "Student Success First",
      title: "Grow Together With Our Community",
      description: "Join a vibrant ecosystem where creativity meets technical excellence and career dreams become reality.",
      offerImage: "https://cdn.pixabay.com/photo/2016/06/06/06/14/limited-time-offer-1438906_1280.png"
    }
  ];
      ...
      <div className="relative z-10 container mx-auto px-8 pt-8 grid grid-cols-1 lg:grid-cols-[1.3fr_0.7fr] items-center gap-12">
        ...
        >
          <div className="inline-flex items-center gap-3 px-4 py-2 rounded-lg bg-primary/10 border border-primary/20 backdrop-blur-xl">
            ...
            <span className="text-[11px] font-bold text-primary tracking-[0.2em]">{slide.tagline}</span>
          </div>

          <h1 className="text-5xl md:text-6xl lg:text-[76px] font-black text-white leading-[0.9] tracking-tight max-w-[12ch] md:max-w-[14ch]">
